import { falso } from "../lib/falso.lib";
import { buildReplacementString, getCircularReplacer, getSecureReplacer, js2SecureJson } from "./secure-json.parser";
...
const prepare = ({
  ...
  const dependencies = {
    ...
    circularReplacer: jest.fn(getCircularReplacer()),
  };
...
it("build-replacement-string", () => {
  ...
  expect(result).toStrictEqual(`_${test.dataset.reason}_<${typeof test.dataset.fieldValue}>${test.dataset.fieldKey}_`);
});
